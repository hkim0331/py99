...
The `home-routes` handler in the `py99.routes.home` namespace
...
The `home-page` function will in turn call the `py99.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `py99.handler` namespace:
...
Request middleware functions are located under the `py99.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `py99.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `py99.db.core` in some other namespace.
