...
1. "Hello, Python!" を画面にプリントしなさい。

1. +, -, *, /, %, //, ** はそれぞれ何を計算する演算子か。

1. 1 > 0 の戻り値は何か？ 2 > 1 の戻り値は何か？戻り値とは何か？

1. <br>a = 1.28 <br>b = 5.4<br>c = (a + b)**2<br>によって c の値は何になるか？

## 統計

1. リスト xs の平均値を返す関数 mean(xs).

1. リスト xs の中央値を返す関数 median(xs).
...
1. 整数 x を引数に取り、それが負数だったら -1, ゼロと等しければ 0, 整数であれば 1 を返す関数 sign(x).
...
1. 整数 n の各桁の総和を返す関数 sum_of_digits(n). sum_of_digits(12345) の戻り値は 15 。
...
1. 600851475143 の素因数のうち最大のものを求めよ。何秒以内？
...
1. 関数 factorial(int n) を定義せよ。factorial(5) は 5! の値を戻り値とする。一般に factoria(n) = n * factorial(n-1).0! は 1 だよ。

1. 0!, 1!, 2! ... を次々に計算していき、n! > m となる最小の n を求める関数int factorial_over(int m). factorial_over(2000000) の戻り値は

1. 0!, 1!, 2! ... を次々に計算していき、n! < 0 となる最小の n を求める関数int factorial_overflow(void).
